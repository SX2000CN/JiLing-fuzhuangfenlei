...
**服装分类系统** 是一个基于PyTorch深度学习的智能服装图片自动分类系统，专门用于电商和服装行业的图片管理。系统能够自动识别并分类服装拍摄照片，支持主图、细节和吊牌三种类别的精准识别。
...
│   │   └── __init__.py
│   ├── gui/                      # 图形界面
│   │   ├── main_window.py        # 传统界面（PySide6）
│   │   └── native_ui.py          # 新界面（VS Code风格）
│   ├── utils/                    # 工具模块
...
### 4. 命令行工具 (`launchers/classify_cli.py`)

**功能**：无GUI的高性能分类工具（**生产环境核心工具**）
**特性**：
- 读取GUI设置（通过QSettings共享路径）
- 自动模型检测
- 批量GPU推理（20线程并行预处理）
- 29.48张/秒处理速度（峰值）
